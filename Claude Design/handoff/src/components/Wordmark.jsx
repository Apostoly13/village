...
/**
 * Wordmark — typographic lockup. Use this in place of image logos for now.
 * Uses Fraunces for the editorial character. Renders crisply at any size.
 *
 * Props:
 *   size      : font-size in px (default 20)
 *   color     : CSS color for "The" (default: var(--ink))
 *   accent    : CSS color for the italic "Village" (default: var(--accent))
 *   invert    : if true, flips to cream text (use on dark hero panels)
 */
export function Wordmark({ size = 20, color, accent, invert = false, style, ...rest }) {
  const c1 = color || (invert ? "var(--brand-cream)" : "var(--ink)");
  const c2 = accent || (invert ? "var(--peach)" : "var(--accent)");
  return (
    <span
      style={{
        fontFamily: "var(--serif)", fontSize: size, fontWeight: 500,
        letterSpacing: "-0.02em", display: "inline-flex", alignItems: "baseline",
        gap: `${size * 0.22}px`, lineHeight: 1, color: c1,
        ...style,
      }}
      {...rest}
    >
...
/** Monogram — single "V" chip for favicons, avatars, watermark corners. */
  ...
  return (
    <span
      ...
      style={{
        ...
        fontSize: size * 0.58, letterSpacing: "-0.04em", lineHeight: 1,
        ...style,
      }}
    >V</span>
  );
